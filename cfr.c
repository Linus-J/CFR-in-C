...
/*TODO:
Unsign ints when possible.
Use hash tables for strategy, strategySum and regret
Look into 7 card hand evals to figure out how to use lookup tables and hash tables
Can make a lookup table for utils


use valgrind with cfr.c
*/
...
int getInfoIndex(char infoStr[MAX_HISTORY_LENGTH]){
    ...
    int index = 0;
    // int infoInts[MAX_HISTORY_LENGTH];
    // printf("length: %d\n", length);
    // for (int i = 0; i<length; i++){
    //     printf("char: %c\n", infoStr[i]);
    //     infoInts[i] = (int)(infoStr[i])-'0';
    //     printf("int: %d\n", infoInts[i]);
    // }
    // printf("woah: %d\n", infoInts[1]);
    switch(length){
...
void compute_avg_strategy(int infoIndex)
{
	...
	//printf("Strat sum: [%f, %f] ", strategy_sum[infoIndex][0], strategy_sum[infoIndex][1]);
    for (int i = 0; i < NUM_ACTIONS; i++){
...
double external_cfr(unsigned short cards[2], char history[MAX_HISTORY_LENGTH], int pot, int traversing_player){
    ...
    //printf("History: %s\n",history);
    /*
        To extend check here if terminal and then change the below to getPayoff function / lookup table.
    */
    if (plays >= 2){ //Check payoff if history not 0, 1
        // printf("first: %c, second: %c\n", history[plays-1], history[plays-2]);
    ...
    char next_history[MAX_HISTORY_LENGTH]={'\0'};
    char temp[MAX_HISTORY_LENGTH]={'\0'};
    ...
    if (acting_player == traversing_player){
        double *tempStrategy;
        tempStrategy = NULL;
        double util[NUM_ACTIONS] = {0}; //2 actions
        double node_util = 0;
        strcpy(tempHistory, history);
        sprintf(cardStr, "%hu", cards[acting_player]);
        int infoIndex = getInfoIndex(strcat(cardStr,tempHistory));
        ...
        sprintf(cardStr, "%hu", cards[acting_player]);
        int infoIndex = getInfoIndex(strcat(cardStr,tempHistory));
        //printf("Infoindex before %d\n",infoIndex);
        compute_strategy(infoIndex,&tempStrategy);
        double tempUtil = 0;
        //char next_history[MAX_HISTORY_LENGTH];
        strcpy(tempHistory, history);
        ...
        strcpy(temp, next_history);
        //printf("nextHIstory2 %s\n",next_history);
        tempUtil = external_cfr(cards, next_history, pot, traversing_player);
        //printf("nextHIstory2 %s DONE\n",next_history);
        //printf("Infoset: %s\n",infoset);
        //printf("strats2: %f, %f\n",tempStrategy[0],tempStrategy[1]);
        //printf("TempUtil: %f\n",tempUtil);
        //printf("CArdstr: %s\n",cardStr);
        //printf("Infoset after: %d\n",infoIndex);
...
void cfr(int iterations)
{
    ...
    for (int t=1; t<iterations + 1; t++){
        ...
        for (int i=0; i<2; i++){ //Number of players is two
            //Shuffle cards
            for (int j=NUM_CARDS; j>1; j--) { //not random
                int p = random_bounded_divisionless(j); // number in [0,i)
                //printf("%d\n",p);
                temp = deck[j-1];
                deck[j-1] = deck[p];
                deck[p] = temp;
                //swap(deck+j-1, deck+p); // swap the values at j-1 and p
            }
            cards[0] = deck[0];
            cards[1] = deck[1];
            //printf("Cards: %d, %d\n", cards[0], cards[1]);
